...
function [results, warp, warpedImage] = ecc(image, template, params)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% False number of input arguments
if nargin<3
...
% Color format validity check for template(RGB or gray-scale)
if sZt3>1
    if ((sZt3==2) || (sZt3>3))
        error('->Unknown color image format: check the number of channels');
    else
...
%% pyramid images
% The following for-loop creates pyramid images in cells IM and TEMP with 
% varying names. The variables IM{1} and TEMP{1} are the images with the 
% highest resoltuion
...
% Resize of images in the pyramid
for nol=2:levels
...
% % Warp initialization for each transformation (if not given)
